<template>
	<div class="dropdown dropdown-end">
		...
		<ul v-if="selectlangName === false" tabindex="0" class="menu dropdown-content p-2 shadow bg-base-100 rounded-box w-52 mt-4 menu-back">
			<li @click="handleCommand('zh')">
				<button class="flex" :class="btuLanguage == 'zh' ? 'buttonColor' : ''">
					<img loading="lazy" width="20" height="20" alt="中文" src="https://cdnjs.cloudflare.com/ajax/libs/twemoji/14.0.0/svg/1f1e8-1f1f3.svg" /> <span class="flex flex-1 justify-between">中文</span>
				</button>
			</li>
			<li @click="handleCommand('en')">
				<button class="flex" :class="btuLanguage == 'en' ? 'buttonColor' : ''">
					<img loading="lazy" width="20" height="20" alt="English" src="https://cdnjs.cloudflare.com/ajax/libs/twemoji/14.0.0/svg/1f1ec-1f1e7.svg" /> <span class="flex flex-1 justify-between">English </span>
				</button>
			</li>
		</ul>
	</div>
